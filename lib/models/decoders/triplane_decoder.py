...
class TriPlaneDecoder(VolumeRenderer):
    ...
    def __init__(self,
        ...
        if disentangle_code:
            base_net_color = []
            for i in range(len(base_layers) - 1):
                base_net_color.append(nn.Linear(base_layers[i], base_layers[i + 1]))
                if i != len(base_layers) - 2:
                    base_net_color.append(activation_layer())
            self.base_net_color = nn.Sequential(*base_net_color)
    ...
    def point_decode(self, xyzs, dirs, code, density_only=False):
        ...
        """
        if len(code.shape)==6 and code.shape[1]==2:
            code_geo = code[:,0]
            code_color = code[:,1]

            num_scenes, _, n_channels, h, w = code_geo.size()

            if self.code_dropout is not None:
                raise NotImplementedError("triplane disentangle do not suppoprt code_dropout")
            if self.scene_base is not None:
                raise NotImplementedError("triplane disentangle do not suppoprt scene_base")
            if isinstance(xyzs, torch.Tensor):
                assert xyzs.dim() == 3
                num_points = xyzs.size(-2)

                point_code_geo = F.grid_sample(
                    code_geo.reshape(num_scenes * 3, -1, h, w),
                    self.xyz_transform(xyzs),
                    mode=self.interp_mode, padding_mode='border', align_corners=False
                ).reshape(num_scenes, 3, -1, num_points)
                point_code_geo = point_code_geo.permute(0, 3, 2, 1).reshape(
                    num_scenes * num_points, -1)

                point_code_color = F.grid_sample(
                    code_color.reshape(num_scenes * 3, -1, h, w),
                    self.xyz_transform(xyzs),
                    mode=self.interp_mode, padding_mode='border', align_corners=False
                ).reshape(num_scenes, 3, -1, num_points)
                point_code_color = point_code_color.permute(0, 3, 2, 1).reshape(
                    num_scenes * num_points, -1)
                ...
                for code_geo_single, code_color_single, xyzs_single in zip(code_geo, code_color, xyzs):
                    num_points_per_scene = xyzs_single.size(-2)

                    # (3, code_chn, num_points_per_scene)
                    point_code_geo_single = F.grid_sample(
                        code_geo_single,
                        self.xyz_transform(xyzs_single),
                        mode=self.interp_mode, padding_mode='border', align_corners=False
                    ).squeeze(-2)
                    point_code_geo_single = point_code_geo_single.permute(2, 1, 0).reshape(
                        num_points_per_scene, -1)

                    point_code_color_single = F.grid_sample(
                        code_color_single,
                        self.xyz_transform(xyzs_single),
                        mode=self.interp_mode, padding_mode='border', align_corners=False
                    ).squeeze(-2)
                    point_code_color_single = point_code_color_single.permute(2, 1, 0).reshape(
                        num_points_per_scene, -1)

                    num_points.append(num_points_per_scene)
                    point_code_geo.append(point_code_geo_single)
                    point_code_color.append(point_code_color_single)

                point_code_geo = torch.cat(point_code_geo, dim=0) if len(point_code_geo) > 1 else point_code_geo[0]
                point_code_color = torch.cat(point_code_color, dim=0) if len(point_code_color) > 1 else point_code_color[0]

            base_x_geo = self.base_net(point_code_geo)
            base_x_act_geo = self.base_activation(base_x_geo)
            sigmas = self.density_net(base_x_act_geo).squeeze(-1)

            # base_x_color = self.base_net(point_code_color)
            base_x_color = self.base_net_color(point_code_color)
            base_x_act_color = self.base_activation(base_x_color)
            ...
            if density_only:
                ...
                if self.use_dir_enc:
                    ...
                    if self.dir_net is not None:
                        color_in = self.base_activation(base_x_color + self.dir_net(sh_enc))
                    else:
                        color_in = torch.cat([base_x_act_color, sh_enc], dim=-1)
                else:
                    color_in = base_x_act_color
    ...
    def visualize(self, code, scene_name, viz_dir, code_range=[-1, 1]):
        if len(code.shape)==6 and code.shape[1]==2:
            num_scenes, _, __, num_chn, h, w = code.size()
            code_viz = code.cpu().numpy()
            if not self.flip_z:
                code_viz = code_viz[..., ::-1, :]
            code_viz = code_viz.transpose(0, 1, 2, 4, 3, 5).reshape(num_scenes, 3 * 2 * h, num_chn * w)
