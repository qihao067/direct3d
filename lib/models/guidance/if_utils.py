...
class IF(nn.Module):
    def __init__(self, device, vram_O, t_range=[0.02, 0.98], recon_loss=False, recon_loss_iter=5000, recon_std_rescale=0.5):
        super().__init__()
    ...
    def train_step_perpneg(self, text_embeddings, weights, pred_rgb, guidance_scale=100, grad_scale=1):
        ...
        if self.recon_loss_enabled:
            ...
            # reconstruct x0
            images_recon = self.predict_start_from_noise(images_noisy, t, noise_pred)

            # clip or rescale x0
            if self.recon_std_rescale > 0:
                images_recon_nocfg = self.predict_start_from_noise(images_noisy, t, noise_pred_text[B:2*B])
                factor = (images_recon_nocfg.std([1,2,3],keepdim=True) + 1e-8) / (images_recon.std([1,2,3],keepdim=True) + 1e-8)
                images_recon_adjust = images_recon.clone() * factor
                images_recon = self.recon_std_rescale * images_recon_adjust + (1-self.recon_std_rescale) * images_recon

            # x0-reconstruction loss from Sec 3.2 and Appendix
            loss = 0.5 * F.mse_loss(images, images_recon.detach(), reduction="sum") / images.shape[0]
