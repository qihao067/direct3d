...
def get_rays(directions, c2w, norm=False, EMpose=False):
    """
    Args:
...
def get_cam_rays(c2w, intrinsics, h, w, EMpose=False):
    directions = get_ray_directions(
        h, w, intrinsics, norm=False, device=intrinsics.device)  # (num_scenes, num_imgs, h, w, 3)
    rays_o, rays_d = get_rays(directions, c2w, norm=True,EMpose=EMpose)
    return rays_o, rays_d
...
class BaseNeRF(nn.Module):
    def __init__(self,
                 ...
                 pretrained=None):
        super().__init__()
        self.disentangle_code = disentangle_code
        if self.disentangle_code:
            code_size_tri = (2,) + code_size_tri
            decoder['disentangle_code'] = True
        else:
            decoder['disentangle_code'] = False

        if code_size_tri is not None:
            self.code_size_tri = code_size_tri
        else:
            self.code_size_tri = code_size
        ...
        if init_from_mean:
            self.register_buffer('init_code', torch.zeros(code_size_tri))
        else:
            self.init_code = None
        ...
        self.decoder_lambda_entropy = decoder_lambda_entropy
    ...
    def get_init_code_(self, num_scenes, device=None):
        code_ = torch.empty(
            self.code_size_tri if num_scenes is None else (num_scenes, *self.code_size_tri),
    ...
    def loss(self, decoder, code, density_bitfield, target_rgbs,
             ...
             rc_only=False, EMpose=False, cfg=dict(), **kwargs):
        if EMpose:
            raise NotImplementedError("We have to remove this part to pass the code check before release.........hmm...")
        ...
        out_weights_sum = outputs['weights_sum']
        out_rgbs = outputs['image'] + self.bg_color * (1 - out_weights_sum.unsqueeze(-1))
        ...
        if self.decoder_lambda_entropy > 0:
            alphas = outputs['weights'].clamp(1e-5, 1 - 1e-5)
            loss_entropy = (- alphas * torch.log2(alphas) - (1 - alphas) * torch.log2(1 - alphas)).mean()
            lambda_entropy = self.decoder_lambda_entropy
            loss = loss + self.decoder_lambda_entropy * loss_entropy
            loss_dict.update(entropy_loss=lambda_entropy * loss_entropy)
    ...
    def loss_decoder(self, decoder, code, density_bitfield, cond_rays_o, cond_rays_d,
                     cond_imgs, dt_gamma=0.0, EMpose=False, cfg=dict(), **kwargs):
        device = code.device
        decoder_training_prev = decoder.training
        decoder.train(True)
        if EMpose:
            num_scenes, num_imgs, h, w, _ = cond_rays_o[list(cond_rays_o.keys())[0]].size()
        else:
        ...
        if EMpose:
            rays_o,rays_d={},{}
            for key in cond_rays_o.keys():
                rays_o[key] = cond_rays_o[key].reshape(num_scenes, num_scene_pixels, 3)
                rays_d[key] = cond_rays_d[key].reshape(num_scenes, num_scene_pixels, 3)
        else:
        ...
        if num_scene_pixels > n_decoder_rays:
            ...
            if EMpose:
                for key in rays_o.keys():
                    rays_o[key] = rays_o[key][scene_arange, inds]  # (num_scenes, n_decoder_rays, 3)
                    rays_d[key] = rays_d[key][scene_arange, inds]  # (num_scenes, n_decoder_rays, 3)
            else:
        ...
        out_rgbs, loss, loss_dict = self.loss(
            ...
            rays_o, rays_d, dt_gamma, return_decoder_loss=True, scale_num_ray=num_scene_pixels, EMpose=EMpose,
            cfg=cfg, **kwargs)
        log_vars = dict()
    ...
    def inverse_code(self, decoder, cond_imgs, cond_rays_o, cond_rays_d, dt_gamma=0, cfg=dict(),
                     ...
                     prior_grad=None, show_pbar=False, EMpose=False):
        """
        ...
        for inverse_step_id in range(n_inverse_steps):
            ...
            out_rgbs, loss, loss_dict = self.loss(
                ...
                target_rgbs, rays_o, rays_d, dt_gamma, scale_num_ray=num_scene_pixels, EMpose=EMpose,
                cfg=cfg)
    ...
    def eval_and_viz(self, data, decoder, code, density_bitfield, viz_dir=None, cfg=dict()):
        ...
        if cfg['img_size']==(256,256):
            assert test_intrinsics[0,0,-1]==128
        else:
            raise NotImplementedError("This code only support render 256 x 256 meshes.")
