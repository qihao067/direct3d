...
class GaussianDiffusion(nn.Module):
    ...
    def pred_x_0(self, x_t, t, grad_guide_fn=None, concat_cond=None, text_cond=None, unconditional_guidance_scale=1., unconditional_conditioning=None, cfg=dict(), update_denoising_output=False):
        if text_cond==None: 
            print("If everything is correct and you are not in the debug mode, then there should be text_cond...  -> gaussian_diffusion.py, Line 182 ")
    ...
    def p_sample_langevin(self,
        ...
        x_0_pred, _ = self.pred_x_0(x_t, t, grad_guide_fn=grad_guide_fn, text_cond=text_cond, unconditional_guidance_scale=unconditional_guidance_scale, 
                                    unconditional_conditioning=unconditional_conditioning, cfg=cfg, **kwargs)
    ...
    def p_sample_ddim(self,
        ...
        x_0_pred, _ = self.pred_x_0(x_t, t, grad_guide_fn=grad_guide_fn, text_cond=text_cond, cfg=cfg, 
                                        unconditional_guidance_scale=unconditional_guidance_scale, unconditional_conditioning=unconditional_conditioning, **kwargs)
    ...
    def ddim_sample(self, noise, show_pbar=False, concat_cond=None, text_cond=None, 
        ...
        for step, t in enumerate(timesteps):
            ...
            x_t, x_0_pred = self.p_sample_ddim(
                x_t, t, t_prev, concat_cond=concat_cond if concat_cond is not None else None,
                unconditional_guidance_scale=unconditional_guidance_scale, unconditional_conditioning=unconditional_conditioning, 
                text_cond=text_cond, cfg=self.test_cfg, **kwargs)
            
            cond_step += 1
            if langevin_steps > 0 and langevin_t_range[0] < t_prev < langevin_t_range[1]:
                for _ in range(langevin_steps):
                    x_t = self.p_sample_langevin(
                        x_t, t_prev, concat_cond=concat_cond if concat_cond is not None else None,
                        text_cond=text_cond, cfg=self.test_cfg, unconditional_guidance_scale=unconditional_guidance_scale, unconditional_conditioning=unconditional_conditioning, **kwargs)
    ...
    def forward_train(self, x_0, concat_cond=None, text_cond=None, grad_guide_fn=None, cfg=dict(),
                      x_t_detach=False, sample_step=-1, **kwargs):
        device = get_module_device(self)
    ...
    def forward_test(self, data, text_cond = None, unconditional_guidance_scale=1., unconditional_conditioning=None, **kwargs):
        ...
        """
        assert data.dim() == 4
        return self.sample_from_noise(data, text_cond=text_cond, unconditional_guidance_scale=unconditional_guidance_scale, unconditional_conditioning=unconditional_conditioning,**kwargs)
