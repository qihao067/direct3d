...
from mmgen.models.architectures.ddpm.modules import TimeEmbedding
...
class DenoisingUnetMod(DenoisingUnet):
    ...
    def __init__(self,
            ...
            for _ in range(resblocks_per_downsample):
                ...
                if scale in attention_scale:
                    if use_text_condition:
                        layers.append(
                            SpatialTransformer(in_channels=in_channels_, n_heads=num_heads, d_head=int(in_channels_/num_heads), use_checkpoint=self.use_checkpoint)
                        )
                    layers.append(
        ...
        # # sum(p.numel() for p in self.mid_blocks.parameters())

    def forward(self, x_t, t, label=None, concat_cond=None, text_cond=None, return_noise=False):
        ...
        for block in self.in_blocks:
            h = block(h, embedding, text_cond)
            hs.append(h)

        # forward middle blocks
        h = self.mid_blocks(h, embedding, text_cond)

        # forward upsample blocks
        for block in self.out_blocks:
            h = block(torch.cat([h, hs.pop()], dim=1), embedding, text_cond)
        outputs = self.out(h)
