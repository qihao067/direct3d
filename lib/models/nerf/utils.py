...
class Trainer(object):
    ...
    ### ------------------------------
    def render_batch_train(self, rays_o, rays_d, mvp, h, w, staged=False, max_ray_batch=4096, **kwargs):
        # rays_o, rays_d: [B, N, 3]
        # return: pred_rgb: [B, N, 3]

        num_scenes = 1
        if 0 < max_ray_batch < rays_o.size(1):
            rays_o = rays_o.split(max_ray_batch, dim=1)
            rays_d = rays_d.split(max_ray_batch, dim=1)
        else:
            rays_o = [rays_o]
            rays_d = [rays_d]
        ...
        for rays_o_single, rays_d_single in zip(rays_o, rays_d):
            outputs = self.model.render(rays_o_single, rays_d_single, mvp, h, w, staged=staged, **kwargs)
            
            weights_sum = outputs['weights_sum']
            weights = outputs['weights']
            rgbs = outputs['image'] # [max_ray_batch, 3]
            depth = outputs['depth']
            out_image.append(rgbs)
            out_depth.append(depth)
            out_weights.append(weights)
            out_weights_sum.append(weights_sum)
        
        out_image = torch.cat(out_image, dim=1) if len(out_image) > 1 else out_image
        out_depth = torch.cat(out_depth, dim=1) if len(out_depth) > 1 else out_depth
        out_weights = torch.cat(out_weights, dim=0) if len(out_weights) > 1 else out_weights
        out_weights_sum = torch.cat(out_weights_sum, dim=1) if len(out_weights_sum) > 1 else out_weights_sum
        
        results = {}
        results['depth'] = out_depth
        results['image'] = out_image
    ...
    def train_step(self, data, save_guidance_path:Path=None):
        ...
        max_ray_batch = self.opt.max_ray_batch
        # outputs = self.render_batch_train(rays_o, rays_d, mvp, H, W, staged=False, max_ray_batch=max_ray_batch, perturb=True, bg_color=bg_color, ambient_ratio=ambient_ratio, shading=shading, binarize=binarize)

        # if isinstance(self.opt.lambda_2d_normal_smooth, list):
        #     if len(self.opt.lambda_2d_normal_smooth) == 4:
        #         lambda_2d_normal_smooth = C(self.opt.lambda_2d_normal_smooth, self.epoch, self.global_step)
        #     else:
        #         lambda_2d_normal_smooth = self.opt.lambda_2d_normal_smooth[0]
        # else:
        #     lambda_2d_normal_smooth = self.opt.lambda_2d_normal_smooth

        outputs = self.model.render(rays_o, rays_d, mvp, H, W, staged=False, perturb=True, bg_color=bg_color, max_ray_batch=max_ray_batch, ambient_ratio=ambient_ratio, shading=shading, binarize=binarize, render_normal=self.opt.render_normal)
        ...
        if isinstance(self.opt.lambda_2d_normal_smooth, list):
            ...
            lambda_2d_normal_smooth = self.opt.lambda_2d_normal_smooth
        if lambda_2d_normal_smooth > 0 and 'normal_image' in outputs:
            ...
            loss = loss + lambda_2d_normal_smooth * loss_smooth
        ...
        # TODO check here whether to use fp32
        if hasattr(self.opt, 'lambda_direct3d_prior'):
            if isinstance(self.opt.lambda_direct3d_prior, list):
                if len(self.opt.lambda_direct3d_prior) == 4:
                    lambda_direct3d_prior = C(self.opt.lambda_direct3d_prior, self.epoch, self.global_step)
                else:
                    lambda_direct3d_prior = self.opt.lambda_direct3d_prior[0]
            elif isinstance(self.opt.lambda_direct3d_prior, float):
                lambda_direct3d_prior = self.opt.lambda_direct3d_prior

            if lambda_direct3d_prior > 0:
                with torch.autocast(
                    device_type='cuda',
                    dtype=torch.float32): # FP16
                    loss_direct3d_prior = self.model.get_direct3d_sds_loss()
                loss = loss + lambda_direct3d_prior * loss_direct3d_prior ##torch.nan_to_num
        
        if getattr(self.opt, 'lambda_init_shape_prior', 0) > 0:
            xyzs = outputs['xyzs']
            dirs = outputs['dirs']
            ...
            #     device_type='cuda',
            #     dtype=torch.float32):
            loss_init_shape_prior = self.model.get_shape_regularization_loss(xyzs, dirs, sigmas)
            
            loss = loss + self.opt.lambda_init_shape_prior * loss_init_shape_prior
        # else:
    ...
    def post_train_step(self):
        ...
        # update guidance
        if 'SD' in self.guidance:
            self.guidance['SD'].update_step(self.epoch, self.global_step)
        elif 'IF' in self.guidance:
            self.guidance['IF'].update_step(self.epoch, self.global_step)

        # if hasattr(self.model, "code_color"):
        #     with torch.no_grad():
        #         # print("code: ", self.model.code)
        #         # print("color code: ", self.model.code_color)
        #         # self.model.code.data.clip(-1, 1)
        #         self.model.code_color.data.clip(-1, 1)
        #         # print("code after clipping: ", self.model.code)
        #         # print("color code after clipping: ", self.model.code_color)

        # clip grad
        if self.opt.grad_clip >= 0:
            # print("code grad: ", self.model.code.grad)
            # print("color code grad: ", self.model.code_color.grad)
            torch.nn.utils.clip_grad_value_(self.model.parameters(), self.opt.grad_clip)
            # print("code grad after clipping: ", self.model.code.grad)
            # print("color code grad after clipping: ", self.model.code_color.grad)
    ...
    def eval_step(self, data):
        ...
        outputs = self.model.render(rays_o, rays_d, mvp, H, W, staged=True, perturb=False, bg_color=None, light_d=light_d, ambient_ratio=ambient_ratio, shading=shading, render_normal=self.opt.render_normal)
    ...
    def save_mesh(self, loader=None, save_path=None):
        ...
        if save_path is None:
            save_path = os.path.join(self.workspace, f'mesh_{self.opt.mcubes_resolution}')
    ...
    def test(self, loader, save_path=None, name=None, write_video=True, save_mesh=True):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader):
                ...
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    # preds, preds_depth, _ = self.test_step(data, bg_color=torch.tensor([1]).type_as(data['rays_o']))
                    preds, preds_depth, _ = self.test_step(data, bg_color=None)
        ...
        if save_mesh and hasattr(self.model, "save_mesh"):
            self.model.save_mesh(self.opt.workspace, ['mesh'])
    ...
    def train_one_epoch(self, loader, max_epochs):
        ...
        loader._data.update_iter(self.global_step)
