...
def rand_poses(size, device, opt, radius_range=[1, 1.5], theta_range=[0, 120], phi_range=[0, 360], return_dirs=False, angle_overhead=30, angle_front=60, uniform_sphere_rate=0.5):
    ...
    right_vector = safe_normalize(torch.cross(forward_vector, up_vector, dim=-1)) # (BUG? this should be left_vector in left-hand coordinate)
    ...
    up_vector = safe_normalize(torch.cross(right_vector, forward_vector, dim=-1) + up_noise) # this is still correct
...
class NeRFDataset:
    def __init__(self, opt, device, type='train', H=256, W=256, size=100, Hs=None, Ws=None, resolution_milestones=[5000]):
        super().__init__()
        ...
        # coarse to fine
        self.Hs = Hs if Hs is not None else [H]
        self.Ws = Ws if Ws is not None else [W]
        self.resolution_milestones = [-1] + resolution_milestones
    ...
    def update_iter(self, global_step: int):
        if len(self.Hs) > 1:
            size_ind = bisect.bisect_right(self.resolution_milestones, global_step) - 1
            self.H = self.Hs[size_ind]
            self.W = self.Ws[size_ind]
            self.cx = self.H / 2
            self.cy = self.W / 2
            print(f"[INFO] set resolution to {self.H}x{self.W}")
    ...
    def collate(self, index):
        ...
        # sample a low-resolution but full image
        # print(f"[INFO] sample rays for resolution {self.H}x{self.W}")
